...
class LoapOptionFilterParser:
    OPERATORS = {
        "contains": {"format": "({} like %({})s)", "extra_value": "%{}%"},
        ...
        if not reqursed:
            if position:
                ...
                for i in position[:-1]:
                    here = here[i]
                if (
                    len(here[position[-1]]) == 3
                    and here[position[-1]][1] != "and"
                    and here[position[-1]][1] != "or"
                ):
                    _format = LoapOptionFilterParser.OPERATORS.get(
                        here[position[-1]][1], {}
                    ).get("format", "")
                    extra_value = LoapOptionFilterParser.OPERATORS.get(
                        here[position[-1]][1], {}
                    ).get("extra_value", "")
                    self.counter += 1
                    if _format:
                        if here[position[-1]][-1] == "":
                            _format = "({} is Null)".format(
                                self.pseudo.get(
                                    here[position[-1]][0], here[position[-1]][0]
                                ),
                                self.counter,
                            )
                        else:
                        ...
                        if extra_value:
                            self.data[str(self.counter)] = extra_value.format(
                                here[position[-1]][2]
                            )
                        else:
                            self.data[str(self.counter)] = (
                                here[position[-1]][2]
                                if here[position[-1]][2] != ""
                                else "Null"
                            )
                    else:
                        _format = "true"
                ...
                if (
                    len(self.original) == 3
                    and self.original[1] != "and"
                    and self.original[1] != "or"
                ):
                    self.counter += 1
                    extra_value = LoapOptionFilterParser.OPERATORS.get(
                        self.original[1], {}
                    ...
                    if extra_value:
                        ...
                        self.data[str(self.counter)] = (
                            self.original[2] if self.original[2] != "" else "Null"
                        )
                    if self.original[2] == "" and self.original[1] == "=":
                        self.original = "({} is Null)".format(
                            self.pseudo.get(self.original[0], self.original[0])
                        )
                    else:
                        self.original = (
                            LoapOptionFilterParser.OPERATORS.get(self.original[1], {})
                            .get("format", "{} {}")
                            .format(
                                self.pseudo.get(self.original[0], self.original[0]),
                                self.counter,
                    ...
                    self.original = "{}={}".format(
                        self.original[0], repr(self.original[1])
                    )
                else:
